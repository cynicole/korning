...
def add_invoice(number, frequency)
  db_connection do |conn|
    invoice_id = conn.exec_params("SELECT id FROM invoices WHERE invoice_no = $1", [number])
    if invoice_id.to_a.empty?
      conn.exec_params("INSERT INTO invoices (invoice_no, frequency) VALUES ($1, $2)", [number, frequency])
    end
  end
end
